...
class User(AbstractUser, BaseModel):
    ...
    profile_pic = models.ImageField(
        upload_to=unique_profile_pic_path, blank=True, null=True
    )
    thumbnail_pic = models.ImageField(
        upload_to="thumbnail_pics/", blank=True, null=True
    )
    ...
    user_type = models.CharField(
        max_length=10, choices=UserTypeChoices, default="customer"
    )
...
class DriverDetail(BaseModel):
    ...
    in_use = models.ForeignKey(
        "vehicle.Vehicle", on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    verification_documents = models.ManyToManyField(
        "DocumentRequired", related_name="driver_detail_documents"
    )
...
class DriverRequest(BaseModel):
    ...
    user = models.ForeignKey(
        ...
        related_name="driver_request_user",
    )
    action_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="driver_request_verifier",
        null=True,
        blank=True,
    )
    dob = models.DateField()
    ...
    status = models.CharField(
        max_length=10, choices=VerificationStatus, default="pending"
    )
    rejection_reason = models.TextField(null=True, blank=True)
    action_at = models.DateTimeField(null=True, blank=True)
    verification_documents = models.ManyToManyField(
        "DocumentRequired", related_name="driver_request_documents"
    )
...
class DocumentRequired(BaseModel):
    ...
    document_image = models.ImageField(
        upload_to=unique_aadhar_photos_path, null=True, blank=True
    )
...
class Trip(BaseModel):
    customer = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="customer_trip"
    )
    ...
    vehicle_type = models.CharField(
        max_length=10, choices=WheelerChoices, null=True, blank=True
    )
    ...
    drop_location_longitude = models.DecimalField(
        max_digits=19, decimal_places=16, null=True, blank=True
    )
    pickup_location_latitude = models.DecimalField(
        max_digits=19, decimal_places=16, null=True, blank=True
    )
    pickup_location_longitude = models.DecimalField(
        max_digits=19, decimal_places=16, null=True, blank=True
    )
    ...
    cancelled_by = models.CharField(
        max_length=20, choices=CancelByStatus, null=True, blank=True
    )
...
class Payment(BaseModel):
    trip = models.ForeignKey(
        Trip, on_delete=models.CASCADE, related_name="trip_payment"
    )
