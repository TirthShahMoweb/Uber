...
class mobileNumberSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not mobile_number.startswith("+"):
            errors = {
                "mobile_number": "Mobile number must start with a '+' sign for the country code."
            }
            raise CustomValidationError(errors)

        if not mobile_number[1:].isdigit():
            errors = {
                "mobile_number": "Mobile number must contain only numeric characters after '+'."
            }
            raise CustomValidationError(errors)
...
class CustomUserSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if len(mobile_number[3:]) != 10:
            errors = {
                "mobile_number": "Mobile number must be exactly 10 digits after the country code."
            }
            raise CustomValidationError(errors)
