...
class VerificationRequestSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if DriverRequest.objects.filter(
            user=self.context["user"], status="pending"
        ).exists():
            errors = {
                "user": "You have already applied as a driver. You cannot apply again."
            }
            raise CustomValidationError(errors)

        if DriverRequest.objects.filter(
            user=self.context["user"], status="approved"
        ).exists():
    ...
    def create(self, validated_data):
        if validated_data["profile_pic"]:
            ...
            if validated_data["profile_pic"].content_type not in [
                "image/jpeg",
                "image/png",
                "image/jpg",
                "image/webp",
            ...
            if field_name in data:
                if field_name == "licence_number":
                    if len(data[field_name]) != 20:
                        errors = {
                            field_name: f"{doc_type.document_label} must be of 20 letters."
                        }
                        raise CustomValidationError(errors)
                ...
                if doc_type.field_type == "image":
                    if type(field_value) is str:
                        raise CustomValidationError(
                            {
                                f"{field_name}": f"{doc_type.document_label} must Required and it should be an image."
                            }
                        )
                    if field_value.content_type not in [
                        "image/jpeg",
                        "image/png",
                        "image/jpg",
                        "image/webp",
                        ...
                        errors = {
                            f"{field_name}": f"{doc_type.document_label} must be an image."
                        }
                        raise CustomValidationError(errors)
                    if field_value.size > 5 * 1024 * 1024:
                        errors = {
                            f"{field_name}": f"{doc_type.document_label} size must be less than or equal to 5 MB."
                        }
                        raise CustomValidationError(errors)
                ...
                if doc_type.field_type == "text":
                    if " " in field_value:
                        errors = {
                            f"{field_name}": f"{doc_type.document_label} does not contain White Space."
                        }
                        raise CustomValidationError(errors)

                    if type(field_value) is not str:
                        errors = {
                            f"{field_name}": f"{doc_type.document_label} must be a string."
                        }
                        raise CustomValidationError(errors)
        ...
        driver = DriverRequest.objects.create(
            user=self.context["user"], dob=validated_data["dob"]
        )
...
class DriverPersonalDetailsViewSerializer(serializers.ModelSerializer):
    ...
    verifier_name = serializers.CharField(
        source="action_by.get_full_name", allow_null=True
    )
