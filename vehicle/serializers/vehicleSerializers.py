...
class VehicleImageSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        vehicle_number = VehicleRequest.objects.all().values_list(
        ...
        if len(vehicle_chassis_number) != 17:
            errors = {
                "vehicle_chassis_number": "Vehicle chassis number must be 17 characters long."
            }
            raise serializers.ValidationError(errors)

        if not vehicle_chassis_number.isalnum():
            errors = {
                "vehicle_chassis_number": "Vehicle chassis number must contain only alphanumeric characters."
            }
            raise serializers.ValidationError(errors)
        ...
        for document_type in required_documents:
            ...
            if data[document_type].content_type not in [
                "image/jpeg",
                "image/png",
                "image/jpg",
                "image/webp",
                ...
                errors = {
                    document_type: f"{document_type} must be a JPEG, PNG, JPG or WEBP image."
                }
                raise serializers.ValidationError(errors)
    ...
    def create(self, validated_data):
        ...
        vehicle_request.vehicle_chassis_number = validated_data[
            "vehicle_chassis_number"
        ]
